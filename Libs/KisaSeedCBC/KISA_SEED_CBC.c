...
int encrytSeedCBC( IN BYTE *pbszPlainText,OUT BYTE *pbszCipherText )
{
    int nPlainTextLen = (int)strlen((char *)pbszPlainText);
    int nClipherTextLen = SEED_CBC_Encrypt(pbszUserKey, pbszIV, pbszPlainText, nPlainTextLen, pbszCipherText);
    return nClipherTextLen;
}

int decrytSeedCBC( IN BYTE *pbszCipherText,OUT BYTE *pbszPlainText )
{
    int nCipherTextLen  = (int)strlen((char *)pbszCipherText);
    int nPlainTextLen   = SEED_CBC_Decrypt(pbszUserKey, pbszIV, pbszCipherText, nCipherTextLen, pbszPlainText);
    return nPlainTextLen;
}


int encrytKeyIVSeedCBC( IN BYTE *pbszUserKey, IN BYTE *pbszIV, IN BYTE *pbszPlainText,OUT BYTE *pbszCipherText )
{
    int nPlainTextLen   = (int)strlen((char *)pbszPlainText);
    int nClipherTextLen = SEED_CBC_Encrypt(pbszUserKey, pbszIV, pbszPlainText, nPlainTextLen, pbszCipherText);
    return nClipherTextLen;
...
int decrytKeyIVSeedCBC( IN BYTE *pbszUserKey, IN BYTE *pbszIV, IN BYTE *pbszCipherText,OUT BYTE *pbszPlainText )
{
    int nCipherTextLen  = (int)strlen((char *)pbszCipherText);
    int nPlainTextLen   = SEED_CBC_Decrypt(pbszUserKey, pbszIV, pbszCipherText, nCipherTextLen, pbszPlainText);
    return nPlainTextLen;
